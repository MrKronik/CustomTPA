        ...
        checkNow(hasUpdate -> {
            if (hasUpdate) {
                Component message = Component.text()
                        .append(Component.text("✨ Доступна новая версия ", TextColor.color(0xFFAA00)))
                        .append(Component.text("CustomTPA " + latestVersion + "! ", TextColor.color(0x55FFFF)))
                        .append(Component.text("Нажмите сюда, чтобы открыть Modrinth", TextColor.color(0x55FF55))
    ...
    public void notifyConsole() {
        checkNow(hasUpdate -> {
            if (hasUpdate) {
                ...
                plugin.getLogger().info("Доступна новая версия CustomTPA: " + latestVersion);
                plugin.getLogger().info("Скачайте на https://modrinth.com/plugin/customtpa");
                plugin.getLogger().info("========================================");
            }
        });
    }
