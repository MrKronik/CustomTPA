...
public class TPAHereCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage(Component.text("Используйте: /tpahere <игрок>"));
            return true;
        }
    ...
    private void sendSenderMessage(Player sender, Player target) {
        sender.sendMessage(config.format(sender, target, "request-here-sent"));
    ...
    private void sendRequestMessage(Player receiver, Player other, boolean isHere) {
        ...
        receiver.sendMessage(base.append(Component.space()).append(acceptBtn).append(Component.space()).append(denyBtn));
    }
