    ...
    private void updateLanguageFiles() {
        ...
        for (String fileName : BUNDLED_LANGUAGES) {
            ...
            if (!file.exists()) {
                ...
                plugin.getLogger().info("Создан языковой файл: " + fileName);
            } else {
                try {
                    ...
                    if (updated) {
                        existing.save(file);
                        plugin.getLogger().info("Обновлён языковой файл: " + fileName + " (добавлены новые ключи)");
                    }
                } catch (Exception e) {
                    plugin.getLogger().warning("Не удалось обновить файл " + fileName + ": " + e.getMessage());
                }
            }
        }
    }
