...
public class TPAcceptCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!player.hasPermission("tpa.accept")) {
            send(player, config.getMessage(player, "no-permission"));
            return true;
        }
        ...
        if (args.length == 0) {
            if (incoming.isEmpty()) {
                send(player, config.getMessage(player, "no-requests"));
                return true;
            }
            ...
            send(player, config.getMessage(player, "prefix").append(Component.text("Запросы: ")).append(list));
            return true;
        }
        ...
        if (request == null) {
            send(player, MiniMessage.miniMessage().deserialize("<red>Запрос от " + name + " не найден.</red>"));
            return true;
        }
        ...
        acceptRequest(plugin, teleporting, other, config);
        return true;
    }

    public static void acceptRequest(TPAPlugin plugin, Player teleporting, Player other, ConfigManager config) {
        int delay = config.getInt("delay-seconds", 3);

        plugin.adventure().player(teleporting).sendMessage(
        ...
        if (delay == 0) {
            teleporting.teleport(other.getLocation());
            plugin.adventure().player(teleporting).sendMessage(config.format(teleporting, other, "request-accepted"));
            plugin.adventure().player(other).sendMessage(config.format(other, teleporting, "request-accepted"));
            return;
        }
            ...
            public void run() {
                if (!teleporting.isOnline() || !other.isOnline()) {
                    if (teleporting.isOnline())
                        plugin.adventure().player(teleporting).sendMessage(config.getMessage(teleporting, "player-offline"));
                    if (other.isOnline())
                        plugin.adventure().player(other).sendMessage(config.getMessage(other, "player-offline"));
                    cancel();
                    return;
                }

                if (hasMoved(teleporting, startLocation)) {
                    plugin.adventure().player(teleporting).sendMessage(config.getMessage(teleporting, "teleport-moved"));
                    cancel();
                    return;
                }
                ...
                teleporting.playSound(teleporting.getLocation(), Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 0.5f, 1.5f);
                ...
                plugin.adventure().player(teleporting).sendActionBar(actionBarMsg);
