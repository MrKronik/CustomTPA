...
public class TPACancelCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (out == null) {
            MessageUtil.send(player, config.getMessage(player, "no-outgoing"));
            return true;
        }
        Player target = Bukkit.getPlayer(out.getTarget());
        if (target != null && target.isOnline()) {
            MessageUtil.send(target, config.format(target, player, "request-cancelled"));
        }
        MessageUtil.send(player, config.format(player, target, "request-cancelled"));
