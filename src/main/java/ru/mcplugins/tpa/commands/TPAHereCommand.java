...
public class TPAHereCommand implements CommandExecutor, TabCompleter {
    ...
    public TPAHereCommand(TPAPlugin plugin, RequestManager requestManager, ConfigManager config) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 0) {
            send(player, Component.text("Используйте: /tpahere <игрок>"));
            return true;
        }
        ...
        if (target.equals(player)) {
            send(player, config.getMessage(player, "self-request"));
            return true;
        }
        if (requestManager.isToggled(target.getUniqueId())) {
            send(player, config.getMessage(player, "toggle-blocked"));
            return true;
        }
        if (requestManager.getRequest(player.getUniqueId(), target.getUniqueId()) != null) {
            send(player, config.getMessage(player, "already-have-request"));
            return true;
        }
    ...
    private void sendSenderMessage(Player sender, Player target) {
        send(sender, config.format(sender, target, "request-here-sent"));
        ...
        send(sender, prompt.append(cancelBtn));
    }
    ...
    private void sendRequestMessage(Player receiver, Player other, boolean isHere) {
        ...
        send(receiver, base);
        send(receiver, acceptPrompt.append(acceptBtn));
        send(receiver, denyPrompt.append(denyBtn));
    }
